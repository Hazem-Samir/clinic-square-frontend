  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit((data) => onSubmit(data.days[0]))} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-1">
          <div className="space-y-2">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('selectDay')}</FormLabel>
        ...
        <Button type="submit" className="w-full">{isLoading?<Spinner/>:(initialData ? t('updateSchedule') : t('addToSchedule'))}</Button>
      </form>
    </Form>
  )
}
      ...
      <BlurFade delay={0} inView>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="font-semibold">{t('sessionCost')}: {cost} EGP</span>
                  ...
                  <div className="space-y-2">
                    {days.map((item) => (
                      <div key={item.day} className="flex justify-between items-center py-2 border-b">
                        ...
                        <span>{HandleTimeFormat(item.startTime)} - {HandleTimeFormat(item.endTime)}</span>
                        <div className="flex space-x-2">
                          <Button disabled={isLoading} variant="outline" size="icon" onClick={() => handleUpdateDay(item)}>
                            <Edit2 className="h-4 w-4" />
                          </Button>
                          <Button disabled={isLoading} variant="outline" size="icon" onClick={() => handleDeleteDay(item)}>
        ...
        <Dialog open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
          <DialogContent>
            ...
            <DialogFooter>
              <Button disabled={isLoading} variant="outline" onClick={() => setIsDeleteModalOpen(false)}>{t('cancel')}</Button>
              <Button variant="destructive" onClick={confirmDelete}>{isLoading?<Spinner />:(t('delete'))}</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={isUpdateModalOpen} onOpenChange={setIsUpdateModalOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t('updateSchedule', { day: currentEditingItem ? t(`days.${currentEditingItem.day}`) : '' })}</DialogTitle>
            </DialogHeader>
            {currentEditingItem && (
              <ScheduleForm
            ...
            <Form {...costForm}>
              <form onSubmit={costForm.handleSubmit(handleSaveSessionCost)} className="space-y-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('sessionCost')}</FormLabel>
                      <FormControl>
                ...
                />
                <Button type="submit" className="w-full">{isLoading?<Spinner/>:(t('setCost'))}</Button>
              </form>
            </Form>
          </DialogContent>
        </Dialog>
