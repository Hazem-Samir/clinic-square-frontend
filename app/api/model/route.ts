...
import { MODEL_URL } from '@/schema/Essentials';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const apiResponse = await fetch(`${MODEL_URL}/questions?populate=patient=name dateOfBirth profilePic gender,answers.doctor=name profilePic specialization&page=${page}&limit=${limit}`, {
      headers: {
    ...
    if (!apiResponse.ok) {
      ...
      export async function POST(request: NextRequest) {
        ...
        try {
          ...
          const apiResponse = await fetch(`${MODEL_URL}/predict`, {
            method: 'POST',
            headers: {
          ...
          const data = await apiResponse.json();
          return NextResponse.json({ success: true, message: 'Model Identified the best specailization', data });
