...
export async function GET(request: NextRequest) {
      ...
      try {
        const apiResponse = await fetch(`${SERVER_URL}/onlinePayment/D-reservation?doctor=${id}&reservationDate=${reservationDate}`, {
          headers: {
  ...
  export async function POST(request: NextRequest) {
    ...
    try {
      ...
      const apiResponse = await fetch(`${SERVER_URL}/lab-Reservation`, {
        method: 'POST',
        headers: {
        'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      const data = await apiResponse.json();
      return NextResponse.json({ success: true, message: 'Cart Ordered Successfully', data });
